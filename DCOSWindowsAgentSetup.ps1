...
Get-DCOSBinaries($download_uri, $download_dir)
{
    ...
    $ret = Invoke-WebRequest -Uri ($download_uri+"/packages.zip") -OutFile ($download_dir+"\packages.zip")
    Expand-ZIPFile -File ($download_dir+"\packages.zip") -Destination $download_dir
